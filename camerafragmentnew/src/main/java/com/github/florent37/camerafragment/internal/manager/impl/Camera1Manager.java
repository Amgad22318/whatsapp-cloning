...
public class Camera1Manager extends BaseCameraManager<Integer, SurfaceHolder.Callback> {
        ...
        backgroundHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.d(TAG, "Can't open fab_camera: " + error.getMessage());
                    if (cameraOpenListener != null) {
                        uiHandler.post(new Runnable() {
                            @Override
                            public void run() {
    ...
    protected void prepareCameraOutputs() {
        try {
            ...
            Log.e(TAG, "Error while setup fab_camera sizes.");
        }
    }
    ...
    protected void releaseVideoRecorder() {
        ...
        try {
            camera.lock(); // lock fab_camera for later use
        } catch (Exception ignore) {
        }
    }
...
//                parameters.setRecordingHint(true);
            ...
            Log.d(TAG, "Error setting fab_camera preview: " + error.getMessage());
        } catch (Exception ignore) {
            Log.d(TAG, "Error starting fab_camera preview: " + ignore.getMessage());
        }
    }
